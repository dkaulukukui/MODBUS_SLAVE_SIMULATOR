// Helper class for Young 41342VC Anemometer with 32400 Serial Interface
// Provides convenient methods to read wind data via ASCII polling over RS-485
// Polling command: Ma! where 'a' is the address (A-Z, 0-9)
// Response format: a,WS,WD,VIN1,VIN2,VIN3,VIN4<CR><LF>
...
public:
  ...
  // Returns wind speed (m/s), wind direction (degrees), and voltage inputs
  bool readAllData(float& windSpeed, float& windDirection, 
                   uint16_t& vin1, uint16_t& vin2, uint16_t& vin3, uint16_t& vin4,
                   uint16_t timeout_ms = 1000);
  
  // Simplified read - just wind speed and direction
  bool readWind(float& windSpeed, float& windDirection, uint16_t timeout_ms = 1000);
...
private:
  ...
  bool parseResponse(const char* response, float& windSpeed, float& windDirection,
                     uint16_t& vin1, uint16_t& vin2, uint16_t& vin3, uint16_t& vin4);
};
